/**
 * ============================================================================
 *  BỆNH VIỆN K — PHÒNG VT-TBYT — HỆ THỐNG GIAO BAN v2.6
...
function _getSupplyRisk(item, idx) {
  ...
  if (!linkedHs.length && keyName) {
    Object.values(idx.hs).forEach(function(h) {
      if (linkedHs.length >= 3) return;
      const hn = _norm(h.ten || "");
      if (hn && hn.indexOf(keyName) >= 0) linkedHs.push(h);
    });
  }
...
// ============================================================================
const _IDX_CACHE_PREFIX = "v26_lidx_";
...
function _buildBriefHtml(data, mode) {
  ...
  const headerLabel = isFlash ? "⚠ CẬP NHẬT NHANH" : "🌅 GIAO BAN SÁNG";
  const headerBg = isFlash ? "linear-gradient(135deg,#9A3412,#DC2626)" : "linear-gradient(135deg,#1E3A8A,#1F4E78)";
...
'.modal-col{border-right:0;border-bottom:1px solid #2D3D5C;padding:14px}\n' +
'.modal-col:last-child{border-bottom:0}\n' +
...
'<header>\n' +
'  <div class="brand-row">\n' +
'    <div class="brand">DASHBOARD GIAO BAN — PHÒNG VT-TBYT<small>Bệnh viện K — 4 tổ: Kỹ thuật / Hồ sơ / Vật tư / Kho · v2.9 · email báo cáo</small></div>\n' +
...
'// v2.7-fix: auto-refresh CHỈ ở tab Overview để không reset filter ở các tab khác.\n' +
'// Người dùng dùng nút 🔄 Refresh khi cần làm mới các tab có filter.\n' +
