...
function getDetail(type, id) {
  ...
  if (type === "KT") {
    ...
    return {
      ...
      vt: linkedVt.concat(fuzzy.vt.filter(f => !linkedVt.some(l => l.obj.ma === f.obj.ma))),
  ...
  if (type === "VT") {
    ...
    const linkedHs = (idx.vtToHs[me.ma] || []).map(m => idx.hs[m]).filter(Boolean)
      .map(o => ({ obj: o, fuzzy: false }));
    return { type: "VT", me: me, sheetUrl: sheetUrl, kt: linkedKt, hs: linkedHs };
  }
  if (type === "HS") {
...
function getLinkedChains() {
  ...
  Object.values(idx.kt).forEach(kt => {
    ...
    chains.push({
      kt: kt, vt: allVt.slice(0, 3), hs: allHs.slice(0, 3),
...
'      html+="<div>";ch.vt.forEach(function(v){html+="<div class=\"chain-node\" data-type=\"VT\" data-id=\""+esc(v.ma||v.ten)+"\" style=\"margin-bottom:4px\"><div class=\"nm\">🧪 "+esc(v.ten)+"</div><div class=\"sm\">"+esc(v.tt||"")+"</div></div>";});html+="</div>";\n' +
