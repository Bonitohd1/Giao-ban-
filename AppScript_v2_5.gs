...
function getVTTH() {
  ...
  // v2.5: build link index 1 lần để enrich kho cho từng dòng VT
  ...
  const rows = t.rows.map((row, i) => {
    ...
    // v2.5: kho mini status — match qua mã VTTH (primary) hoặc tên loại (fallback)
    obj.kho = _resolveKhoForVt({ ma: obj.ma, ten: obj.loai }, linkIdx);
    return obj;
  }).filter(r => r);
...
function getKho() {
  ...
  // v2.5: enrich từng dòng 5A với (queue 5B + VT linked + HS linked) để render expand inline
  const linkIdx = _buildLinkIndex();
  ton.forEach(function(r) {
    const keyMa = (r.ma || "").toString().toLowerCase().trim();
    const keyName = _norm(r.ten || "");
    let queue = [];
    if (keyMa && linkIdx.kho5b[keyMa]) queue = linkIdx.kho5b[keyMa].slice();
    if (!queue.length && keyName && linkIdx.kho5bByName[keyName]) queue = linkIdx.kho5bByName[keyName].slice();
    ...
    r.queue = queue;
    r.queueOpen = queue.filter(function(q) { return (q.tt || "").indexOf("Đã cấp đủ") < 0; }).length;

    const linkedVt = [];
    if (keyMa) {
      Object.values(linkIdx.vt).forEach(function(v) {
        if ((v.ma || "").toLowerCase() === keyMa) linkedVt.push(v);
      });
    }
    if (!linkedVt.length && keyName) {
      const vByName = linkIdx.vtByName[keyName];
      if (vByName) linkedVt.push(vByName);
      if (!linkedVt.length) {
        const tokens = keyName.split(/\s+/).filter(function(t) { return t.length > 3; });
        Object.values(linkIdx.vtByName).forEach(function(v) {
          if (linkedVt.length >= 5) return;
          const vn = _norm(v.ten || "");
          if (tokens.some(function(t) { return vn.indexOf(t) >= 0; })) linkedVt.push(v);
        });
      }
    }
    r.vt = linkedVt.slice(0, 5);

    const hsSet = {};
    const linkedHs = [];
    r.vt.forEach(function(v) {
      const hsList = linkIdx.vtToHs[v.ma] || [];
      hsList.forEach(function(maHs) {
        const hs = linkIdx.hs[maHs];
        if (hs && !hsSet[hs.ma]) { hsSet[hs.ma] = 1; linkedHs.push(hs); }
      });
    });
    r.hs = linkedHs.slice(0, 5);
  });

  dexuat.forEach(function(r) {
    const keyMa = (r.ma || "").toString().toLowerCase().trim();
    const keyName = _norm(r.vtth || "");
    let vt = null;
    if (keyMa) Object.values(linkIdx.vt).forEach(function(v) { if (!vt && (v.ma || "").toLowerCase() === keyMa) vt = v; });
    if (!vt && keyName) vt = linkIdx.vtByName[keyName] || null;
    r.vt = vt;
    r.hs = vt ? (linkIdx.vtToHs[vt.ma] || []).map(function(m) { return linkIdx.hs[m]; }).filter(Boolean).slice(0, 3) : [];
  });
...
'<header>\n' +
'  <div class="brand-row">\n' +
'    <div class="brand">DASHBOARD GIAO BAN — PHÒNG VT-TBYT<small>Bệnh viện K — 4 tổ: Kỹ thuật / Hồ sơ / Vật tư / Kho · v2.5 · liên kết Kho ↔ VT ↔ HS</small></div>\n' +
...
'      html+="<div class=\\"chain-row\\">";\n' +
'      html+="<div class=\\"chain-title\\"><span class=\\"severity "+sev+"\\">"+sevLbl+"</span>";\n' +
'      html+="<div style=\\"color:#fff;font-weight:600\\">📦 "+esc(ch.kho.ten||"?")+(ch.kho.ma?" ("+esc(ch.kho.ma)+")":"")+"</div>";\n' +
'      var doh=ch.kho.doh;\n' +
...
'    });\n' +
'    html += "<div class=\\"tbl-wrap\\"><table class=\\"tbl\\"><thead><tr><th>Loại</th><th>Khoa</th><th>Cơ sở</th><th>CB</th><th>Trạng thái</th><th>📦 Kho</th><th>Tiến độ</th><th>%</th><th>Deadline</th><th>Vướng mắc</th><th>HT</th></tr></thead><tbody>";\n' +
'    rows.forEach(function(r){\n' +
'      var dl = r.dl + (r.tre?" <span class=\\"tre-badge\\">Trễ "+r.tre+"n</span>":"");\n' +
'      var pillU = r.ut.toLowerCase().indexOf("cao")>=0?"red":"gray";\n' +
'      var pct = r.pct!==null?(r.pct+"%"):"-";\n' +
'      var khoCell = "";\n' +
'      if(r.kho && r.kho.stock){\n' +
'        var sevK = khoSeverity(r.kho.stock);\n' +
'        khoCell = "<span class=\\"mini-pill stock-pill "+sevK+"\\" title=\\"Tồn: "+esc(r.kho.stock.ton||"-")+" · DOH: "+esc(r.kho.stock.doh||"-")+"\\">"+esc((r.kho.stock.tt||"?").substring(0,8))+"</span>";\n' +
'        if(r.kho.queue && r.kho.queue.length){\n' +
'          var openK = r.kho.queue.filter(function(q){return(q.tt||"").indexOf("Đã cấp đủ")<0;}).length;\n' +
'          if(openK)khoCell += " <span class=\\"mini-pill stock-pill orange\\">"+openK+" khoa chờ</span>";\n' +
'        }\n' +
'      } else if(r.kho && r.kho.queue && r.kho.queue.length){\n' +
'        var openO = r.kho.queue.filter(function(q){return(q.tt||"").indexOf("Đã cấp đủ")<0;}).length;\n' +
'        khoCell = "<span class=\\"mini-pill stock-pill orange\\">"+openO+" khoa chờ</span>";\n' +
'      } else {\n' +
'        khoCell = "<span style=\\"color:#4B5563;font-size:11px\\">—</span>";\n' +
'      }\n' +
'      html += "<tr data-type=\\"VT\\" data-id=\\""+esc(r.ma||r.loai)+"\\" data-tab=\\""+esc(TAB_NAME.VT)+"\\" data-gid=\\""+r.gid+"\\" data-row=\\""+r.idx+"\\">";\n' +
